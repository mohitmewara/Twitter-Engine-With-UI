...
var sendtweetlist = $('#send-tweet-list');
var tweetbtn = $('#sendtweetbtn');
var tweetbox = $('#tweetbox');
var followbtn = $('#follow');
var followtxt = $('#followtext');
var homepagediv = $('#homepage');
var searchhashtagtext = $('#searchhashtag');
var searchhashtagbtn = $('#searchhashtagbtn');

var searchusernametxt = $('#searchusername');
var searchusernamebtn = $('#searchusernamebtn');

var retweetusernametxt = $('#retweetusername');
var retweettxt = $('#retweettxt');
var retweetbtn = $('#retweetbtn');

var loggedinuser = name.val();

searchhashtagbtn.on('click', function (event) {
  channel.push('search_hashtag', { hashtag: searchhashtagtext.val() });
});

searchusernamebtn.on('click', function (event) {
...
tweetbtn.on('click', function (event) {
  channel.push('send_tweet', { name: name.val(), tweet: tweetbox.val() });
});
...
followbtn.on('click', function (event) {
  channel.push('follow', { following: followtxt.val(), follower: name.val() });
});

channel.on('receive_tweet', function (payload) {
  list.append("<b>" + (payload.name || 'Celebrity') + " tweeted : </b> " + payload.message + "<br>");
  list.prop({ scrollTop: list.prop("scrollHeight") });
});

channel.on('receive_retweet', function (payload) {
  list.append("<b>" + (payload.username1 || 'Celebrity') + " retweeted " + payload.username2 + " tweet - </b> " + payload.message + "<br>");
  list.prop({ scrollTop: list.prop("scrollHeight") });
});

channel.on('receive_response', function (payload) {
